...
void onedim_binpack(heurpair *i, heurpair *f, heurpair *l, int *b, int bno,
		itype *z) {
	int j, *bc;
	heurpair *k;
...
void dfirst_heuristic(allinfo *a) {
	...
	for (f = a->fbox, l = a->lbox, h = 0;; h++) {
		...
		f = remboxes(f, l, &d);
		t[h].d = d;
		t[h].bno = h + 1; /* initially put layers into separate bins */
		t[h].z = 0;
		t[h].lno = h + 1; /* this ensures fes. solution if terminate */
	}

	/* split into bins by solving 1-dim binpacking */
	for (i = 0; i < h; i++)
		b[i] = a->D; /* all bins are empty */
	...
	if (z < a->zlayer)
		a->zlayer = z;
	if (a->zlayer < a->z)
		savesol(a, a->fbox, a->lbox, a->zlayer);
}
...
void recpack(allinfo *a, int i, int j, int n, box *f, int rel) {
	...
	if (stopped)
		return;
	a->iter3d++;
	if ((a->iter3d == a->maxiter) && (a->maxiter != 0))
		terminate = TRUE;
	a->subiterat++;
...
void envelope(point *f, point *l, point *s1, point **sm, itype W, itype H,
		itype D, itype RW, itype RH, itype cz, point **ll, int *nz, stype *area) {
	...
	for (i = t = f, s = s1; i != l; i++) {
		...
		if (iz <= cz)
			continue;
		if (iz < mz)
			mz = iz; /* find minimum next z coordinate */
		ix = i->x;
		if (ix <= x) {
			if (iz > z) {
				*t = *i;
...
void initboxes(box *f, box *l, point *fc, point **lc, int *minw, int *minh,
		int *mind) {
	register box *j, *m;
	register point *k;
...
stype findplaces(allinfo *a, box *f, box *l, point *s1, point **sm, stype fill) {
	register point *k;
	...
	if (lc >= fc)
		psortdecr(fc, lc); /* order decreasing */
	...
	for (z = 0; z != D; z = zn) {
		...
		if (zn + mind > D)
			zn = D; /* nothing fits between zn and D */
		...
		if (bound > a->maxfill) {
			...
			for (s = s1; s != sl + 1; s++) {
				for (i = f, d = 0; i != l + 1; i++) {
					...
					d++;
					if (d == a->mcut)
						break; /* terminate after mcut branches */
					if (terminate)
						break;
				}
			}
...
boolean onebin_robot(allinfo *a, box *f, box *l, boolean fast) {
	...
	if (a->maxfill == a->BVOL) { /* NB: maxfill = BVOL, when optimal found */
		for (i = a->fsol, j = f, m = l + 1; j != m; i++, j++)
			*j = *i;
		return TRUE;
	}
	return FALSE;
...
void mcut_heuristic(allinfo *a) {
	...
	if (b < a->zmcut)
		a->zmcut = b; /* save solution */
	if (a->zmcut < a->z)
		savesol(a, a->fbox, a->lbox, a->zmcut);
}
...
boolean fits3(box *i, box *j, box *k, itype W, itype H, itype D, int packtype) {
	...
	for (r = 1; r <= 3; r++) {
		...
		h = H - k->h;
		d = D - k->d;
		if ((i->w <= w) && (j->w <= w) && fits2(i, j, w, H, D)) {
			k->x = w;
			return TRUE;
		}
		if ((i->h <= h) && (j->h <= h) && fits2(i, j, W, h, D)) {
			k->y = h;
			return TRUE;
		}
		if ((i->d <= d) && (j->d <= d) && fits2(i, j, W, H, d)) {
			k->z = d;
			return TRUE;
		}
		t = i;
	...
	if ((packtype != GENERAL) && (packtype != ROBOT))
		return FALSE;
...
boolean onebin_decision(allinfo *a, box *j, int bno) {
	...
	*k = *j;
	...
	switch (size) {
	case 0:
		error("no boxes in onebin_decision");
	case 1:
		fits = TRUE;
		k->x = k->y = k->z = 0;
		break;
	case 2:
		fits = fits2(t, k, a->W, a->H, a->D);
		break;
	case 3:
		fits = fits3(t, t + 1, k, a->W, a->H, a->D, a->packtype);
		break;
	default:
		fits = fitsm(a, t, k, FALSE);
		break;
	}
	if (size <= 3) {
		a->subiterat++;
...
boolean onebin_heuristic(allinfo *a, box *f, box *l) {
	...
	switch (DIF(f,l)) {
	case 0:
		error("no boxes in onebin_heuristic");
	case 1:
		fits = TRUE;
		break;
	case 2:
		fits = fits2(f, l, a->W, a->H, a->D);
		break;
	case 3:
		fits = fits3(f, f + 1, l, a->W, a->H, a->D, a->packtype);
		break;
	default:
		fits = fitsm(a, f, l, TRUE);
		break;
	}
	return fits;
}
...
boolean try_close(allinfo *a, box **curr, ntype bno, box *oldf, box **oldl,
		box **oldlc, ntype *oldnoc, boolean *oldclosed, int level) {
...
void free_close(allinfo *a, ntype bno, box *oldf, box *oldl, box *oldlc,
		ntype oldnoc, boolean *oldclosed) {
	a->lbox = oldl;
	a->lclosed = oldlc;
	a->noc = oldnoc;
...
void rec_binpack(allinfo *a, box *i, int bno, ntype lb, int level) {
	...
	if (i == a->lbox + 1) {
		...
		if (i == a->lbox + 1) { /* all boxes went into closed bins */
			...
			if (more)
				lb = a->noc + bound_two(a, a->fbox, a->lbox);
			if (lb < a->z) {
				for (b = 1; b <= bno; b++) {
					if (a->closed[b])
						continue; /* cannot add to closed bin */
		...
		/* restore */
		if (more)
			free_close(a, bno, of, ol, ox, oc, ocl);
	}
}
...
void binpack3d(int n, int W, int H, int D, int *w, int *h, int *d, int *x,
