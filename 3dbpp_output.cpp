...
void output::save_packlist() {
	packlist_vector.push_back(_packlist);
	_packlist.clear();
}

void output::exportpl() {
	for (uint i = 0; i < packlist_vector.size(); i++) {
		packlist_ packlist = packlist_vector[i];
		char f_s[100];
		sprintf(f_s, "%s/pack_list_%d.txt", dir.c_str(), i);
...
void output::savepl_xml() {
	...
	for (uint pn = 0; pn < packlist_vector.size(); pn++) {
		packlist_ packlist = packlist_vector[pn];
...
double output::find_com_height() {
	...
	for (uint i = 0; i < _packlist.size(); i++) {
		mass += _packlist[i].get_weight();
		h += _packlist[i].get_weight() * ((_packlist[i].get_height() / 2.0)
				+ h_prev);
		h_prev += _packlist[i].get_height();
	}
...
void output::run_mcmc(int iterations) {
	...
	for (int i = 0; i < n; i++) {
		pos.push_back(i);
		for (int j = i; j < n; j++) {
			if (_packlist[i].get_weight() > _packlist[j].get_weight()) {
				t[i][j] = 8;
				t[j][i] = 2;
			} else if (_packlist[i].get_weight() < _packlist[j].get_weight()) {
				t[i][j] = 2;
				t[j][i] = 8;
			}
		}
	...
	while (iterations > 0) {
		...
		swap(_packlist[e1], _packlist[e2]);
		for (int i = 0; i < n; i++) {
			double temp = t[e1][i];
			t[e1][i] = t[e2][i];
			t[e2][i] = temp;
		...
		if ((pl_new < pl) && !(coin_flip(pl_new / pl))) {
			swap(_packlist[e2], _packlist[e1]);
			for (int i = 0; i < n; i++) {
				double temp = t[e1][i];
				t[e1][i] = t[e2][i];
				t[e2][i] = temp;
